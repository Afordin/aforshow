      ...
      <div className="flex justify-center items-start">
        <div className="w-full sm:w-10/12 md:w-1/2">
          ...
          <ul className="flex flex-col">
            { faq.map((item, index) => {
              return (
              <li key={ index } className={`bg-transparent my-2 shadow-lg border-clip-text gradient rounded ${openFaq}`} onClick={() => handleFaq(index)}>
                <h2
                  className="flex flex-row text-white justify-between items-center font-semibold p-3 cursor-pointer"
                >
                  <span>{item.question}</span>
                  <svg
                    className={`fill-current text-purple-700 h-6 w-6 transform transition-transform duration-500 ${openFaq === index ? 'rotate-180' : ''}`}
                    viewBox="0 0 20 20"
                  >
                ...
                <div
                  className={`border-l-2 border-purple-600 overflow-hidden duration-500 transition-all ${openFaq === index ? '' : 'hidden'}`}
                >
